...
(function () {
    ...
    Chart.prototype._initialize = function () {
        ...
        for (series in this._options.data) {
            ...
            if (dataSeries.markerSize === null) {
                if ((dataSeries.type === "line" && dataSeries.dataPoints.length < this.canvas.width / 16) || dataSeries.type === "scatter") {
    ...
    Chart._supportedChartTypes = ["line", "column", "area", "bar", "bubble", "scatter",
    ...
    Chart.prototype._processData = function () {
        ...
        for (var i = 0; i < this.plotInfo.plotTypes.length; i++) {
            ...
            for (var j = 0; j < plotType.plotUnits.length; j++) {
                ...
                if (plotUnit.type === "line" || plotUnit.type === "column" || plotUnit.type === "area" || plotUnit.type === "bar" || plotUnit.type === "bubble" || plotUnit.type === "scatter")
    ...
    Chart.prototype._mouseEventHandler = function (ev) {
        ...
        if (!this.isDrag || !this.zoomEnabled) {
    ...
    Legend.prototype.render = function () {
        ...
        for (var i = 0; i < this.dataSeries.length; i++) {
            ...
            var markerType = dataSeries.legendMarkerType ? dataSeries.legendMarkerType : (dataSeries.type === "line" || dataSeries.type === "scatter" || dataSeries.type === "bubble") && dataSeries.markerType ? dataSeries.markerType : DataSeries.getDefaultLegendMarker(dataSeries.type);
            ...
            var markerSize = (!dataSeries.markerSize && dataSeries.type === "line") ? 0 : this.fontSize * .7;
            ...
            if (dataSeries.type !== "pie" && dataSeries.type !== "doughnut") {
                ...
                for (var dataPointIndex = 0; dataPointIndex < dataSeries.dataPoints.length; dataPointIndex++) {
                    ...
                    var markerColor = dataPoint.markerColor ? dataPoint.markerColor : dataSeries.markerColor ? dataSeries.markerColor : dataPoint.color ? dataPoint.color : dataSeries.color ? dataSeries.color : dataSeries._colorSet[dataPointIndex];
                    var markerSize = ((dataPoint.markerSize === 0 || (dataSeries.markerSize === 0 && !dataPoint.markerSize)) && dataSeries.type === "line") ? 0 : this.fontSize * .7;
    ...
    DataSeries.prototype.getDefaultAxisPlacement = function () {
        ...
        if (type === "column" || type === "line" || type === "area" || type === "stackedcolumn" || type === "stackedline" || type === "bubble" || type === "scatter"
    ...
    DataSeries.getDefaultLegendMarker = function (type) {
        ...
        else if (type === "line" || type === "pie" || type === "doughnut") {
            return "circle";
        } else if (type === "area" || type === "stackedarea" || type === "stackedarea100") {
            return "triangle"
        } else {
            window.console.log("Unknown Chart Type: " + type);
            return null;
    ...
    Axis.prototype.createLabels = function () {
        ...
        if (this.type === "axisX" && this.chart.plotInfo.axisXValueType === "dateTime") {
            ...
            if (Math.floor(this.interval) < this.interval && !this._options.interval) {//Check if it should be rendered as a category axis. If yes, then ceil the interval
    ...
    Axis.setLayoutAndRender = function (axisX, axisY, axisY2, axisPlacement, freeSpace) {
        ...
        if (axisPlacement === "normal") {
            ...
            ctx.rect(plotArea.x1 - Math.max(axisYlineThickness + axisX.gridThickness) / 2,
            ...
            ctx.save();
            ctx.rect(plotArea.x1 - Math.max(axisX.lineThickness, axisYGridThickness, axisY2GridThickness),
                plotArea.y1,
                Math.abs(plotArea.x2 - plotArea.x1 + Math.max(axisX.lineThickness, axisYGridThickness, axisY2GridThickness) + Math.max(axisYGridThickness, axisY2GridThickness)),
                Math.abs(plotArea.y2 - plotArea.y1 + (axisYlineThickness ? axisYlineThickness / 2 : 0)));
    ...
    ToolTip.prototype._updateToolTip = function (mouseX, mouseY) {
        ...
        if (this.shared) {
            ...
            if (this.currentSeriesIndex >= 0) {
                ...
                } else if (dataSeries.type === "line" || dataSeries.type === "area" || dataSeries.type === "stackedArea" || dataSeries.type === "stackedArea100") {
        ...
        if (entries.length > 0) {
            ...
            this.contentDiv.style.borderColor = this.borderColor ? this.borderColor : entries[0].dataPoint.color ? entries[0].dataPoint.color : entries[0].dataSeries.color ? entries[0].dataSeries.color : entries[0].dataSeries._colorSet[entries[0].index % entries[0].dataSeries._colorSet.length];
            ...
            if (entries.length === 1 && !this.shared && (entries[0].dataSeries.type === "line" || entries[0].dataSeries.type === "area" || entries[0].dataSeries.type === "stackedArea" || entries[0].dataSeries.type === "stackedArea100")) {
    ...
    ToolTip.prototype.highlightObjects = function (entries) {
        ...
        for (var i = 0; i < entries.length; i++) {
            ...
            if (dataSeries.type === "line" || dataSeries.type === "scatter"
                || dataSeries.type === "area" || dataSeries.type === "stackedArea" || dataSeries.type === "stackedArea100") {
        ...
        if (isToolTipDefinedInData && this.content && typeof (this.content) === "function") {
            ...
            if (entries.length > 1) {
                ...
                for (var i = 0; i < entries.length; i++) {
                    ...
                    if (dataSeries.type === "line" || dataSeries.type === "area" || dataSeries.type === "column" || dataSeries.type === "bar" || dataSeries.type === "scatter"
